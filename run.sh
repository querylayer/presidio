...
docker run --rm --name presidio-analyzer --network $NETWORKNAME -d -p 3000:3000 \
-e GRPC_PORT=3000 \
-e RECOGNIZERS_STORE_SVC_ADDRESS=presidio-recognizers-store:3004 \
-e NLP_CONF_PATH=${NLP_CONF_PATH} \
-e ENABLE_TEXT_ANALYTICS_RECOGNIZER=True \
-e TEXT_ANALYTICS_ENDPOINT=https://skyfer-pii-detector.cognitiveservices.azure.com \
-e TEXT_ANALYTICS_KEY=9d5287acaea6414c9206942062ef0bee \
${DOCKER_REGISTRY}/presidio-analyzer:${PRESIDIO_LABEL}
...
docker run --rm --name presidio-api --network $NETWORKNAME -d \
-p 8080:8080 \
-e WEB_PORT=8080 \
-e ANALYZER_SVC_ADDRESS=presidio-analyzer:3000 -e ANONYMIZER_SVC_ADDRESS=presidio-anonymizer:3001 \
-e RECOGNIZERS_STORE_SVC_ADDRESS=presidio-recognizers-store:3004 \
${DOCKER_REGISTRY}/presidio-api:${PRESIDIO_LABEL}
